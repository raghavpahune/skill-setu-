...
export function formatStudentProfilePayload(form) {
  ...
  const gradYear = form.graduation_year ? parseInt(form.graduation_year, 10) : null;
  ...
  return {
    ...
    graduation_year: Number.isInteger(gradYear) && gradYear >= 1970 && gradYear <= 2100 ? gradYear : null,
...
export function formatEmployeeProfilePayload(form) {
  ...
  const yoe = form.years_of_experience !== undefined && form.years_of_experience !== null ? parseFloat(form.years_of_experience) : 0;

  return {
    current_role: (form.current_role || '').trim(),
    years_of_experience: !Number.isNaN(yoe) && yoe >= 0 ? yoe : 0,
...
export function validateEmployeeProfile(payload) {
  ...
  if (payload.years_of_experience < 0 || Number.isNaN(payload.years_of_experience)) {
