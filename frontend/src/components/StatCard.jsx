...
export default function StatCard({
  ...
  const colorStyles = {
    ...
    amber: 'bg-amber-50/80 dark:bg-amber-950/40 text-amber-950 dark:text-amber-200 border-amber-300 dark:border-amber-700/80',
  ...
  return (
    <div
      className={`p-4 sm:p-5 rounded-xl border transition-all duration-200 shadow-xs hover:shadow-sm ${
        dominant ? 'ring-2 ring-amber-500/40 dark:ring-amber-400/30' : ''
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {badge && (
              <span className="text-[9px] font-mono font-bold uppercase px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-500/30">
                {badge}
              </span>
            )}
          </div>
          <p
            className={`${
              dominant ? 'text-3xl sm:text-4xl font-black' : 'text-2xl sm:text-3xl font-extrabold'
            } mt-1 tracking-tight`}
          >
            {value}
          </p>
      ...
      {(subtitle || trend) && (
        <div className="mt-2.5 flex items-center gap-2 text-xs flex-wrap">
          ...
          {subtitle && <span className="opacity-75 text-[11px] font-medium line-clamp-1">{subtitle}</span>}
        </div>
      )}
    </div>
  );
