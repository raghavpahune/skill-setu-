...
export default function RecommendationCard({ rec }) {
  ...
  return (
    <div className="p-5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs hover:border-teal-300 dark:hover:border-teal-700 transition-all">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center justify-center w-5 h-5 rounded-md bg-teal-100 dark:bg-teal-950 text-teal-800 dark:text-teal-300 text-xs font-bold shrink-0">
              💡
            </span>
        ...
        {rec.confidence && (
          <div className="text-right shrink-0">
            <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 block uppercase">Confidence</span>
            <span className="inline-block px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 font-bold text-xs border border-emerald-200 dark:border-emerald-800 mt-0.5 font-mono">
              {rec.confidence}%
            </span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 text-xs flex-wrap">
          ...
          {rec.future_demand && (
            <span className="px-2 py-0.5 bg-teal-50 dark:bg-teal-950 text-teal-800 dark:text-teal-300 rounded font-medium border border-teal-200 dark:border-teal-800">
              Trend: <strong className="capitalize">{rec.trend || 'rising'}</strong>
            </span>
          )}
        </div>
      ...
      {expanded && (
        <div className="mt-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-300 space-y-1.5 animate-fadeIn">
